...
type Formulario = {
  ...
  gerador_marca: string; gerador_modelo: string; gerador_numero_serie: string; esp32_modelo: string; esp32_ip: string; modbus_protocolo: string; modbus_baudrate: string; modbus_servidor: string; modbus_paridade: string; modbus_stop_bits: string; foto_url: string;
};
const vazio: Formulario = { gerador_local: '', fuel: '', usca_modelo: '', tensao_ac: '', tensao_dc: '', corrente: '', kva: '', fabricado: '', filtro_combustivel: '', filtro_oleo: '', filtro_agua: '', filtro_ar: '', motor_marca: '', motor_modelo: '', motor_numero_serie: '', gerador_marca: '', gerador_modelo: '', gerador_numero_serie: '', esp32_modelo: '', esp32_ip: '', modbus_protocolo: 'Modbus RTU', modbus_baudrate: '19200', modbus_servidor: '10', modbus_paridade: '', modbus_stop_bits: '', foto_url: '' };
...
export default function HomePage() {
  ...
  async function remover(g: Gerador) { if (!window.confirm('Desativar ' + g.identificacao + '?')) return; setErro(''); try { await desativarGerador(g.id); setGeradores((atuais) => atuais.filter((atual) => atual.id !== g.id)); await carregar(); } catch (error) { setErro(error instanceof Error ? error.message : 'Não foi possível desativar'); } }
  ...
  return <main className="dashboard-shell">
    ...
    {aberto && <div className={styles.modalBackdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="form-title"><div className={styles.modalHeader}><div><span className={styles.tableKicker}>DADOS DO ATIVO</span><h2 id="form-title">{editando ? 'Editar gerador' : 'Novo gerador'}</h2></div><button className={styles.closeButton} onClick={() => setAberto(false)} aria-label="Fechar">×</button></div><div className={styles.modalPhoto}>{formulario.foto_url ? <img src={resolverFotoUrl(formulario.foto_url) || ""} alt="Previa da foto do gerador" /> : <span>Nenhuma foto selecionada</span>}</div><form className={styles.generatorForm} onSubmit={salvar}>
  ...
  <div className={styles.formSection}><h3>COMUNICAÇÃO ESP32 / DSE</h3><label>MÓDULO ESP32<input value={formulario.esp32_modelo} onChange={(e) => alterar('esp32_modelo', e.target.value)} placeholder="HF-006Enet" /></label><label>IP ETHERNET<input value={formulario.esp32_ip} onChange={(e) => alterar('esp32_ip', e.target.value)} placeholder="A definir" /></label><label>PROTOCOLO<input value={formulario.modbus_protocolo} onChange={(e) => alterar('modbus_protocolo', e.target.value)} placeholder="Modbus RTU" /></label><label>BAUD RATE<input value={formulario.modbus_baudrate} onChange={(e) => alterar('modbus_baudrate', e.target.value)} placeholder="19200" /></label><label>ID ESCRAVO DSE<input value={formulario.modbus_servidor} onChange={(e) => alterar('modbus_servidor', e.target.value)} placeholder="10" /></label><label>PARIDADE<input value={formulario.modbus_paridade} onChange={(e) => alterar('modbus_paridade', e.target.value)} placeholder="A confirmar" /></label><label>STOP BITS<input value={formulario.modbus_stop_bits} onChange={(e) => alterar('modbus_stop_bits', e.target.value)} placeholder="A confirmar" /></label></div>
