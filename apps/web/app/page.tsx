...
type Formulario = {
  ...
  gerador_marca: string; gerador_modelo: string; gerador_numero_serie: string; esp32_modelo: string; esp32_rede: string; esp32_ip: string; esp32_mac: string; esp32_firmware: string; modbus_protocolo: string; modbus_baudrate: string; modbus_servidor: string; modbus_paridade: string; modbus_stop_bits: string; modbus_pinagem: string; modbus_terminacao: string; foto_url: string;
};
const vazio: Formulario = { gerador_local: '', fuel: '', usca_modelo: '', tensao_ac: '', tensao_dc: '', corrente: '', kva: '', fabricado: '', filtro_combustivel: '', filtro_oleo: '', filtro_agua: '', filtro_ar: '', motor_marca: '', motor_modelo: '', motor_numero_serie: '', gerador_marca: '', gerador_modelo: '', gerador_numero_serie: '', esp32_modelo: '', esp32_rede: '', esp32_ip: '', esp32_mac: '', esp32_firmware: '', modbus_protocolo: 'Modbus RTU', modbus_baudrate: '19200', modbus_servidor: '10', modbus_paridade: '', modbus_stop_bits: '', modbus_pinagem: '56 blindagem ? 57 B+ ? 58 A-', modbus_terminacao: '120 ? nas extremidades', foto_url: '' };
...
export default function HomePage() {
  ...
  <div className={styles.formSection}><h3>GATEWAY ESP32 / MODBUS</h3><label>MODELO DO M?DULO<input value={formulario.esp32_modelo} onChange={(e) => alterar('esp32_modelo', e.target.value)} placeholder="HF-006Enet" /></label><label>REDE<input value={formulario.esp32_rede} onChange={(e) => alterar('esp32_rede', e.target.value)} placeholder="Ethernet" /></label><label>IP ETHERNET<input value={formulario.esp32_ip} onChange={(e) => alterar('esp32_ip', e.target.value)} placeholder="A definir" /></label><label>MAC<input value={formulario.esp32_mac} onChange={(e) => alterar('esp32_mac', e.target.value)} placeholder="A definir" /></label><label>FIRMWARE<input value={formulario.esp32_firmware} onChange={(e) => alterar('esp32_firmware', e.target.value)} placeholder="A definir" /></label><label>PROTOCOLO<input value={formulario.modbus_protocolo} onChange={(e) => alterar('modbus_protocolo', e.target.value)} placeholder="Modbus RTU" /></label><label>BAUD RATE<input value={formulario.modbus_baudrate} onChange={(e) => alterar('modbus_baudrate', e.target.value)} placeholder="19200" /></label><label>ID DO SERVIDOR<input value={formulario.modbus_servidor} onChange={(e) => alterar('modbus_servidor', e.target.value)} placeholder="10" /></label><label>PARIDADE<input value={formulario.modbus_paridade} onChange={(e) => alterar('modbus_paridade', e.target.value)} placeholder="A confirmar" /></label><label>STOP BITS<input value={formulario.modbus_stop_bits} onChange={(e) => alterar('modbus_stop_bits', e.target.value)} placeholder="A confirmar" /></label><label>PINAGEM RS485<input value={formulario.modbus_pinagem} onChange={(e) => alterar('modbus_pinagem', e.target.value)} placeholder="56 / 57 / 58" /></label><label>TERMINA??O<input value={formulario.modbus_terminacao} onChange={(e) => alterar('modbus_terminacao', e.target.value)} placeholder="120 ? nas extremidades" /></label></div>
