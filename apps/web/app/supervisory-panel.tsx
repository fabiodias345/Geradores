...
function ModbusTab({ gerador, atualizar }: { gerador: Gerador; atualizar: (g: Gerador) => void }) {
  ...
  const [form, setForm] = useState({ modulo: ler(gerador, 'esp32_modelo') === '—' ? 'HF-006Enet' : ler(gerador, 'esp32_modelo'), ip: ler(gerador, 'esp32_ip'), protocolo: ler(gerador, 'modbus_protocolo') === '—' ? 'Modbus RTU' : ler(gerador, 'modbus_protocolo'), baudrate: ler(gerador, 'modbus_baudrate') === '—' ? '19200' : ler(gerador, 'modbus_baudrate'), servidor: ler(gerador, 'modbus_servidor') === '—' ? '10' : ler(gerador, 'modbus_servidor'), paridade: ler(gerador, 'modbus_paridade') === '—' ? 'A confirmar' : ler(gerador, 'modbus_paridade'), stopBits: ler(gerador, 'modbus_stop_bits') === '—' ? 'A confirmar' : ler(gerador, 'modbus_stop_bits') });
  async function salvar() { setSalvando(true); setErro(''); try { const resultado = await atualizarGerador(gerador.id, { dados_tecnicos: { ...gerador.dados_tecnicos, esp32_modelo: form.modulo, esp32_ip: form.ip, modbus_protocolo: form.protocolo, modbus_baudrate: form.baudrate, modbus_servidor: form.servidor, modbus_paridade: form.paridade, modbus_stop_bits: form.stopBits } }); atualizar(resultado.gerador); } catch (e) { setErro(e instanceof Error ? e.message : 'Não foi possível salvar a configuração Modbus'); } finally { setSalvando(false); } }
  const campos: [keyof typeof form, string][] = [['modulo', 'Módulo ESP32'], ['ip', 'IP Ethernet'], ['protocolo', 'Protocolo'], ['baudrate', 'Baud rate'], ['servidor', 'ID escravo DSE'], ['paridade', 'Paridade'], ['stopBits', 'Stop bits']];
  return <section className={styles.modbusCard}><div className={styles.cardHeading}><div><p className={styles.kicker}>COMUNICAÇÃO DO GERADOR</p><h2>ESP32 + DSE7320</h2></div><span>Somente leitura nesta fase</span></div><div className={styles.modbusNotice}>Este módulo pertence a este gerador. Ele será mestre Modbus do controlador local e enviará a telemetria pela Ethernet. Nenhum comando de partida ou parada é habilitado.</div><div className={styles.modbusForm}>{campos.map(([campo, label]) => <label key={campo}>{label}<input value={form[campo]} onChange={(e) => setForm((atual) => ({ ...atual, [campo]: e.target.value }))} /></label>)}</div>{erro && <p className={styles.modbusError}>{erro}</p>}<button className={styles.saveModbus} onClick={salvar} disabled={salvando}>{salvando ? 'Salvando...' : 'Salvar dados do ESP32'}</button></section>;
}
